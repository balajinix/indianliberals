...
def select_to_write(handle, page_name, page_id, text, upload_link_url):
  ...
  try:
    ...
    # idea is to read tweets from publish_tweets file and write to output files, from where they'll be pickeup up
    if get_char == 'y' or get_char == 't':
        if get_char == 't':
            text = raw_input("New title: ")
            text = text.strip()
        tweet_file = "../output/tweets/" + handle + ".txt"
        ...
        try:
            ...
            tf.close()
        # temp
        temp_file = "../output/temp_tweets/" + handle + ".txt"
        tf = open(temp_file, "a+")
        try:
            output = text + "~" + upload_link_url + "~" + page_name + "~" + page_id + "\n"
            tf.write(output)
